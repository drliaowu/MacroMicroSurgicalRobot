...
CurrPose = geometry_msgs.Pose(geometry_msgs.Vector3(-0.4, -0.1, 0.4), geometry_msgs.Quaternion(0.707, 0.707, 0, 0))
...
class TrajectoryClient:
    ...
    def send_cartesian_trajectory(self, pose: geometry_msgs.Pose):
...
def callback(data: OmniState):
    rospy.loginfo(rospy.get_caller_id() + f'Position: {data.pose.position.x}, {data.pose.position.y}, {data.pose.position.z}')

if __name__ == '__main__':
    rospy.init_node('ur5e_control_py', anonymous=True)
